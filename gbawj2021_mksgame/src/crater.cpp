...
namespace mks
{
    ...
    void Crater::init(bn::random& random, bn::fixed_point& new_position)
    {        
        MapEntity::init();

        position = new_position;
        angle = random.get_fixed(bn::fixed(0), bn::fixed(360));

        int sprite_index = random.get_int(0, MAX_CRATER_INDEX);
    ...
    void Crater::update(VectorHelper& vector_helper, bn::fixed_point& map_center, bn::fixed& map_yaw)
    {
        MapEntity::update();
